      ...
      <tbody>
        ...
        <tr v-for="item in _filtered_list" :key="item.id">
          <td class="clickable w3-hover-pale-blue" @click="emitEvent('toggle', item)">
            <i class='bx bx-square' v-if="item.status == 'not_started'"></i>
            <i class='bx bxs-minus-square' v-if="item.status == 'in_progress'"></i>
            <i class='bx bxs-check-square' v-if="item.status == 'completed'"></i>
          </td>
          <td>{{ item.text }}</td>
